...
export function PuzzleHubScreen() {
  ...
  if (isLoading) {
    return (
      <FeatureScreenLayout title={t('puzzle.puzzles')} subtitle={t('puzzle.sharpen_skills')} statsRow={null}>
        <FeatureCard
          icon="loader"
      ...
      statsRow={
        puzzleStats && (
          <VStack gap={4}>
            ...
            <HStack gap={3}>
              <StatCard icon="flame" value={puzzleStats.currentStreak.toString()} label={t('puzzle.streak')} />
              <StatCard
...
export default PuzzleHubScreen;
