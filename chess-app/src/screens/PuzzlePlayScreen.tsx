...
import { PuzzleApiClient, PuzzleAttempt } from '../api/puzzleApi';
...
const PuzzlePlayScreen: React.FC<PuzzlePlayScreenProps> = ({ 
  puzzleId, 
  onComplete,
  ...
  useEffect(() => {
    const fetchPuzzleData = async () => {
      try {
        ...
        const puzzleData = await client.getPuzzle(puzzleId);
  ...
  const handleSubmit = async () => {
    try {
      ...
      const result = await client.submitAttempt(puzzleId, attempt);
  ...
  return (
    <View style={styles.container} testID="puzzle-play-screen">
      <Text style={styles.header} testID="puzzle-header">Daily Puzzle · {puzzle.date || 'Today'}</Text>
      <Text style={styles.rating} testID="tactics-rating">Tactics: {puzzle.rating || 'N/A'}</Text>
